 ...
 */
const EMPTY = { dismissedAt: null, signedUp: false };
function storage() {
 ...
 */
export function wasShownThisSession(namespace) {
...
export function markShownThisSession(namespace) {
    const store = sessionStore();
    if (!store)
        return;
    try {
...
export function readSuppression(namespace) {
    const store = storage();
    if (!store)
        return EMPTY;
    try {
        ...
        return {
            dismissedAt: typeof parsed.dismissedAt === 'number' && Number.isFinite(parsed.dismissedAt)
                ? parsed.dismissedAt
                : null,
...
function write(namespace, state) {
    const store = storage();
    if (!store)
        return;
    try {
...
export function recordDismissal(namespace, now = Date.now()) {
    write(namespace, { ...readSuppression(namespace), dismissedAt: now });
}
export function recordSignup(namespace, now = Date.now()) {
    write(namespace, { dismissedAt: now, signedUp: true });
}
/** True when suppression says we must stay quiet right now. */
export function isSuppressed(state, dismissDays, now = Date.now()) {
    if (state.signedUp)
        return true;
